...
// START of assignment code section
    ...
    if (err != 0){
        fprintf(stderr, "Error creating mutex; code %d", err);
        exit(EXIT_FAILURE);
    }
...
// START of assignment code section 
    ...
    if (err != CL_SUCCESS){
        fprintf(stderr,"Error creating input1 buffer; code %d", err);
    ...
    if (err != CL_SUCCESS){
        fprintf(stderr, "Error creating input2 buffer; code %d", err);
        pthread_mutex_unlock(&(ocl->device_lock));
        return *status;
    }
    ...
    if (err != CL_SUCCESS){
        fprintf(stderr, "Error creating output buffer; code %d", err);
        pthread_mutex_unlock(&(ocl->device_lock));
        return *status;
    }

    status_buf = clCreateBuffer(ocl->context, CL_MEM_READ_ONLY, sizeof(status), NULL, &err);
    if (err != CL_SUCCESS){
        fprintf(stderr, "Error creating status buffer; code %d", err);
        pthread_mutex_unlock(&(ocl->device_lock));
        return *status;
    }
    ...
    for (int i = 0; i < max_iters; i++){
        ...
        if (err != CL_SUCCESS){
            fprintf(stderr, "Error enqueueing input buffer; code %d", err);
            pthread_mutex_unlock(&(ocl->device_lock));
            return *status;
        }
        ...
        if (err != CL_SUCCESS){
            fprintf(stderr, "Error setting kernel arg; code %d", err);
            pthread_mutex_unlock(&(ocl->device_lock));
            return *status;
        }
        ...
        if (err != CL_SUCCESS){
            fprintf(stderr, "Error enqueueing commands; code %d", err);
            pthread_mutex_unlock(&(ocl->device_lock));
            return *status;
        }
        ...
        if (err != CL_SUCCESS){
            fprintf(stderr, "Error creating buffer; code %d", err);
            pthread_mutex_unlock(&(ocl->device_lock));
            return *status;
        }
    ...
    if(status[0] != 0){
        fprintf(stderr, "Error max iterations reached");
    }
    ...
    if (err != CL_SUCCESS){
        fprintf(stderr, "Error releasing object; code %d", err);
        pthread_mutex_unlock(&(ocl->device_lock));
        return *status;
    }
